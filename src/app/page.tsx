...
export default function Home(username: String) {
  const user  = getUserGitHub(username);
    if (user != null) {
      return (
        <section className="flex min-h-screen items-center justify-center bg-gray-950">
          <div className="bg-gray-50 p-6 flex flex-col">
            ...
            <Link href="/usuario/[user]" as={`/usuario/${user}`} className="bg-purple-700 hover:bg-purple-500 transition-colors duration-100 text-center p-2 rounded">
              Ver perfil
            </Link>
          </div>
        </section >
      ...
      return (
        <section className="flex min-h-screen items-center justify-center bg-gray-950">
          <div className="bg-gray-50 p-6 flex flex-col">
            <h2 className="text-gray-900 text-lg font-bold">Usuário não encontrado</h2>
          </div>
        </section>
      );
    }
